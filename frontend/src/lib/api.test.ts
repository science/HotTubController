...
describe('API Client', () => {
	...
	it('heaterOn calls correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledWith('/api/equipment/heater/on', { method: 'POST' });
	...
	it('heaterOff calls correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledWith('/api/equipment/heater/off', { method: 'POST' });
		expect(result.success).toBe(true);
	...
	it('pumpRun calls correct endpoint', async () => {
		...
		expect(mockFetch).toHaveBeenCalledWith('/api/equipment/pump/run', { method: 'POST' });
		expect(result.success).toBe(true);
